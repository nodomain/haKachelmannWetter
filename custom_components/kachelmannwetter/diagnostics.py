...
async def async_get_config_entry_diagnostics(
    ...
    if CONF_API_KEY in config_data:
        ...
        config_data[CONF_API_KEY] = f"{key[:8]}...{key[-4:]}" if len(key) > 12 else "***"
    ...
    return {
        ...
        "forecast_daily_first": data.get("forecast_daily", [None])[0]
        if data.get("forecast_daily")
        else None,
        "forecast_hourly_first": data.get("forecast_hourly", [None])[0]
